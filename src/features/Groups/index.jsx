...
function Groups(props) {
  ...
  const handleSubmit = async (values) => {
    const data = {
      ...values,
      members: [...members, { id: user.id, gmail: user.gmail, role: "Owner" }],
    };
    try {
  ...
  return (
    <div className={classes.root}>
      <Box
        className={classes.BtnGroupMoblie}
        sx={{
          width: "100%",
          height: "50px",
          display: "flex",
      ...
      >
        ...
        <Dialog open={open} onClose={handleCloseCreateGroup}>
          ...
          <form onSubmit={form.handleSubmit(handleSubmit)}>
            ...
            <DialogContent>
              <DialogContentText>
                You can add members to your group by entering the gmail address you need to add.
              </DialogContentText>
              <InputField label="Name's Group" name='name_group' form={form} />
              <TextAreaField label='Describe about group' name='describe_group' form={form} />
              <Autocomplete
                ...
                renderInput={(params) => (
                  <TextField {...params} margin='normal' label='Add member' placeholder='Gmail' />
                )}
